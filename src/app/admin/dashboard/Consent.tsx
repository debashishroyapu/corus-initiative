...
export default function Consent() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        setLoading(true);
        // ✅ /api prefix সরানো, safeGet সরাসরি ConsentStatsData return করে
        const data = await safeGet<ConsentStatsData>('/admin/consent/stats');
        if (data?.summary) {
          setStats({
            total: data.summary.totalConsent,
            accepted: data.summary.acceptedConsent,
            declined: data.summary.declinedConsent,
            acceptanceRate: data.summary.acceptanceRate.toFixed(1),
          });
          setError(null);
        } else {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
      ...
      {loading && <div className="text-gray-500 text-center py-8">Loading consent data...</div>}
